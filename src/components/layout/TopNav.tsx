  ...
  return (
    <header className="h-14 bg-white dark:bg-slate-900/90 border-b border-slate-200 dark:border-slate-800/80 px-4 sm:px-6 flex items-center justify-between sticky top-0 z-30 backdrop-blur transition-colors">
      ...
      <div className="flex items-center gap-3">
        ...
        {/* Dynamic Authenticated Role Indicator / Switcher */}
        <div className="relative">
          {user && user.roles && user.roles.length > 1 ? (
            <button
              ...
              title="Click to Switch Between Your Authorized Workspaces"
            >
              ...
              <span className="text-[9px] px-1 py-0.2 bg-blue-100 dark:bg-blue-950 text-blue-700 dark:text-blue-300 font-bold rounded">
                {user.roles.length} Workspaces
              </span>
              <ChevronDown className="w-3 h-3 text-slate-400" />
            </button>
          ) : (
            <div
              className="flex items-center gap-1.5 px-2.5 py-1 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded text-xs text-slate-700 dark:text-slate-200 font-medium"
              title={`Single Authorized Workspace: ${roleMeta.title} (No arbitrary role selection permitted)`}
            >
              <ShieldCheck className="w-3.5 h-3.5 text-blue-600 dark:text-blue-400" />
              <span className="font-mono text-[11px] font-semibold">{roleMeta.title}</span>
              <span className="text-[9px] px-1 py-0.2 bg-emerald-100 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-300 font-mono font-semibold rounded">
                Governed
              </span>
            </div>
          )}

          {showRoleMenu && user && user.roles && user.roles.length > 1 && (
            <div className="absolute right-0 mt-1.5 w-72 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg shadow-xl py-1.5 z-50 text-xs font-sans">
                  ...
                  >
                    <div>
                      <div className="flex items-center gap-1.5">
                        <span className="font-medium text-slate-900 dark:text-white">{meta.title}</span>
                      ...
                      <span className="text-[10px] text-slate-500 dark:text-slate-400">{meta.focus}</span>
                    </div>
              ...
              <div className="border-t border-slate-100 dark:border-slate-800 mt-1 pt-1 px-2">
                ...
                >
                  ...
                  <span>Switch Officer Account / Parichay SSO</span>
                </button>
              </div>
            </div>
          )}
