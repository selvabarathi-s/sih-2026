...
import { Search, Bell, ShieldCheck, UserCheck, RefreshCw, Sun, Moon, Database, LogIn, LogOut, ChevronDown, Check, KeyRound } from 'lucide-react';
...
export const TopNav: React.FC = () => {
  ...
  const roleMeta = (currentRole && ROLE_METADATA[currentRole]) ? ROLE_METADATA[currentRole] : ROLE_METADATA.monitoring_officer;
  ...
  const isProjectScoped = Boolean(user && user?.assignedProjects && user.assignedProjects.length > 0);
  const isMinistryScoped = Boolean(user && currentRole === 'admin_ministry_review');
  ...
  return (
    <header className="h-16 bg-white dark:bg-slate-900/95 border-b border-slate-200 dark:border-slate-800 px-4 sm:px-6 flex items-center justify-between sticky top-0 z-30 backdrop-blur transition-colors">
      ...
      <div className="flex items-center gap-2 sm:gap-2.5 shrink-0">
        ...
        {!user ? (
          /* Public / Unauthenticated State: Prominent Official Login Button */
          <button
            onClick={() => navigate('/login')}
            className="flex items-center gap-2 px-3.5 py-1.5 bg-blue-600 hover:bg-blue-500 text-white rounded-lg text-xs font-bold font-mono transition shadow-sm cursor-pointer ml-1"
            title="Official Government & Stakeholder Login"
          >
            <KeyRound className="w-3.5 h-3.5" />
            <span>Official Login / Roles</span>
          </button>
        ) : (
          /* Authenticated Officer State: Workload Bell + Officer Profile Card */
          <>
            ...
            <div className="relative" ref={menuRef}>
              ...
              >
                <div className="w-8 h-8 rounded-md bg-gradient-to-br from-blue-600 to-indigo-700 text-white flex items-center justify-center font-bold text-xs shadow-sm shrink-0 font-mono">
                  {user?.fullName ? user.fullName.split(' ').map((n: string) => n[0]).join('').slice(0, 2) : 'OF'}
                </div>
              ...
              {showRoleMenu && (
                <div className="absolute right-0 mt-2 w-80 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-2xl py-2 z-50 text-xs font-sans">
                  ...
                  <div className="border-t border-slate-100 dark:border-slate-800 p-2 bg-slate-50/50 dark:bg-slate-950/50 space-y-1.5">
                    <button
                      onClick={() => {
                        setShowRoleMenu(false);
                        navigate('/login');
                      }}
                      className="w-full py-1.5 px-3 text-center rounded-lg bg-blue-600 hover:bg-blue-500 text-white font-semibold text-xs flex items-center justify-center gap-2 transition shadow-sm cursor-pointer"
                    >
                    ...
                    <button
                      ...
                      className="w-full py-1.5 px-3 text-center rounded-lg bg-rose-50 dark:bg-rose-950/50 hover:bg-rose-100 dark:hover:bg-rose-900/50 text-rose-700 dark:text-rose-300 border border-rose-200 dark:border-rose-800 font-semibold text-xs flex items-center justify-center gap-1.5 transition cursor-pointer"
                    >
                      <LogOut className="w-3.5 h-3.5" />
                      <span>Sign Out (Return to Public Portal)</span>
                    </button>
                  </div>
                </div>
              )}
