...
Runs all 21 comprehensive suites with 100% verification coverage.
"""
...
def run_all_tests():
    ...
    try:
        ...
        print("  ✓ Detected active service running on http://127.0.0.1:3000")
    except Exception:
        print("  * Starting test application server on http://127.0.0.1:3000...")
        env = os.environ.copy()
        env["PORT"] = "3000"
        env["NODE_ENV"] = "test"
    ...
    try:
        ...
        # 2. Ingestion & Mathematical Reconciliation Suite
        ...
        assert ret.returncode == 0, "Ingestion and reconciliation tests failed!"

        # 3. Dataset Modes & Architectural Isolation Suite
        ...
        assert ret.returncode == 0, "Dataset mode isolation tests failed!"

        # 4. Theme & High-Contrast Gov Mode Suite
        ...
        assert ret.returncode == 0, "Theme tests failed!"
        ...
        assert ret.returncode == 0, "Auth and RBAC tests failed!"
        ...
        # 7. Stage 3 Multi-Role Workflows Suite
        ...
        assert ret.returncode == 0, "Stage 3 workflow tests failed!"
        ...
        # 10. Stage 4 Weak Signals & Anomaly Suite
        ...
        # 21. 18-Role Real-World Operational Architecture Suite
        print("\n--- Running 18-Role Operational Architecture & Multi-Assignment Suite ---")
        ret = subprocess.run([sys.executable, os.path.join(os.path.dirname(__file__), 'verify_18_roles.py')])
        assert ret.returncode == 0, "18-Role verification tests failed!"
